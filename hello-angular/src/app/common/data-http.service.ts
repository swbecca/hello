...
export class dataHttpService {
    ...
    dataConfigs: any = DataConfigService;
                    ...
                    if (api.method == "post") {
                        data[name] = function (param, header) {
                            if (!header || (header && !header.headers)) {
                                let headers:any = {};
                                if ((<any>window).sessionStorage.hospitalStr) {
                                    headers = new HttpHeaders().set('Authorization', th.CookieService.getCookie('token')).set('campus', (<any>window).sessionStorage.hospitalStr);
                                }else{
                                    headers = new HttpHeaders().set('Authorization', th.CookieService.getCookie('token'))
                                }
                                header = {
                                    headers: headers
                                }
